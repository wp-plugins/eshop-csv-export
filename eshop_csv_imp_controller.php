...
if ( !class_exists( 'eshop_csv_imp' ) ) {
	...
	class eshop_csv_imp {
		...
		function eshop_csv_imp( ) { // Constructor
			$this->view = new view( );
			$this->csv = new CSV( );
		...
		function eshop_csv_imp_admin_pages( ) {
		...
		function getExportLink( $subdir, $filename ) {
			...
			if ( $this->csv->saveToFile( $csv_data, $filename, WP_CONTENT_DIR . $subdir ) ) {
				$url = WP_CONTENT_URL . $subdir . '/' . $filename . '.csv';
				update_post_meta( 1, '_eshop_csv_imp_backup', $url );
			} else {
				$url = FALSE;
			}
			return $url;
...
// Instantiate
if (class_exists("eshop_csv_imp")) {
	$eshop_csv_imp = new eshop_csv_imp();
}

// Initialize the admin panel
if (!function_exists("eshop_csv_imp_ap")) {
	function eshop_csv_imp_ap() {
		global $eshop_csv_imp;
		if (!isset($eshop_csv_imp)) {
			return;
		}
		if (function_exists( 'add_submenu_page' ) ) {
			add_submenu_page( 'tools.php', __( 'eShop CSV Export' ), __( 'eShop CSV Export' ), 'administrator', basename(__FILE__), array(&$eshop_csv_imp, 'eshop_csv_imp_admin_pages'));
		}
	}	
}
...
if ( !function_exists( "ecsvi_header" ) ) {
	function ecsvi_header( ) {
		$ecsvi_url = get_bloginfo('wpurl') . '/wp-content/plugins/eshop-csv-import/css/eshop_csv_imp.css';
...
//Actions and Filters	
if ( isset( $eshop_csv_imp ) ) {
	add_action( 'admin_menu', 'eshop_csv_imp_ap' );
